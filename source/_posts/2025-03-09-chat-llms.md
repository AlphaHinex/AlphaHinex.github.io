...
description: "一个可以与多个 LLM 进行一问多答的命令行工具"
date: 2025.03.09 10:26
categories:
    - Go
    - AI
tags: [Go, Golang, AI]
keywords: cli, chat-llms, OpenAI, chat, chat completions, DeepSeek
cover: /contents/covers/chat-llms.png
---

![](/contents/covers/chat-llms.png)

用法
----

- 在 [Releases](https://github.com/AlphaHinex/go-toolkit/releases) 页面下载对应平台的二进制文件。
- 生成配置文件模板
    ```bash
    ./chat-llms -t 
    ```
...
- 执行对话，等待结果生成
    ...
    ./chat-llms
    ```
- 默认每个模型每个温度分别调用3次流式和非流式接口，调用结果默认生成到 `./result` 路径下。
